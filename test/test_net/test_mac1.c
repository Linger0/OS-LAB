...
uint32_t buffer[PSIZE] = {0xffffffff, 0x5500ffff, 0xf77db57d, 0x00450008, 0x0000d400, 0x11ff0040, 
    0xa8c073d8, 0x00e00101, 0xe914fb00, 0x0004e914, 0x0000, 0x005e0001, 0x2300fb00, 0x84b7f28b, 
    0x00450008, 0x0000d400, 0x11ff0040, 0xa8c073d8, 0x00e00101, 0xe914fb00, 0x0801e914, 0x0000};
...
static void mac_send_desc_init(mac_t *mac)
{
    ...
    for (i = 0; i < PNUM; i++) {
        send_desc[i].tdes0 = 0;
        send_desc[i].tdes1 = (1 << 24) | BYTE_CNT;
    ...
    send_desc[PNUM - 1].tdes1 = (1 << 25) | (1 << 24) | BYTE_CNT;
...
static void mac_recv_desc_init(mac_t *mac)
{
    ...
    for (i = 0; i < PNUM; i++) {
        ...
        receive_desc[i].tdes1 = (1 << 24) | BYTE_CNT;
...
/*
    uint32_t *addr = P2V(SEND_PHY_ADDR);
    sys_move_cursor(1, 3);
    printf("%x %x %x %x", *addr, *(addr + 1), *(addr + 2), *(addr + 3));
    sys_move_cursor(1, 4);
    print_tx_dscrb(&test_mac);
*/
